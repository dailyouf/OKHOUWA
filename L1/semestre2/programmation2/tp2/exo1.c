...
int ecart_dates_rec_ter_aux(struct date date1, struct date date2, int n, int signe);
...
int main(){
	...
	printf("Le nombre de jours entre ces deux dates avec recusivité terminale est de : %d jours \n", ecart_dates_rec_ter(date1, date2));
...
int ecart_dates_rec_ter_aux(struct date date1, struct date date2, int n, int signe){
	...
	if (camparer_date(date1, date2) > 0){
		return ecart_dates_rec_ter_aux(date2, date1, n, -1 * signe);
	}
		
	date1  = lendemain(date1);
	
	return ecart_dates_rec_ter_aux(date1, date2, ++n, signe);
}
